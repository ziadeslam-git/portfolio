...
const Education = () => {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const dots = gsap.utils.toArray('.education-timeline-dot');
  ...
  return (
    <section id="education" ref={containerRef} className="py-24 relative overflow-hidden bg-transparent z-10">
                ...
                return (
                  <div key={item.id} className={`relative flex flex-col md:flex-row items-center ${isEven ? 'md:flex-row' : 'md:flex-row-reverse'} justify-center w-full`}>
                    ...
                    <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 w-8 h-8 items-center justify-center z-10">
                      <div className="education-timeline-dot w-3 h-3 rounded-full bg-white/20 transition-colors duration-300"></div>
                    </div>
